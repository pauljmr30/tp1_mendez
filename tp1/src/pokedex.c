...
{
    struct pokedex *pokedex = (struct pokedex *)malloc(sizeof(struct pokedex));
    ...
    pokedex->pokemons = (struct pokemon*) malloc(sizeof(struct pokemon) * (pokedex->cantidad_pokemons));
...
const struct pokemon *pokedex_buscar_pokemon(struct pokedex *pokedex, const char *nombre)
{
    ...
    for(size_t i = 0; i < pokedex->cantidad_pokemons; i++){
        ...
        for(size_t j = 0; j < str_lenr(pokedex->pokemons[i].nombre, 0); j++){

            if(strings_iguales(nombre, pokedex->pokemons[i].nombre)){
...
void pokedex_ordenar_pokemons_alfabeticamente(struct pokedex *pokedex){
    ...
    while((pokemons_no_ordenados > 1)){
        ...
        for(size_t i = 1; i < pokemons_no_ordenados; i++){
            if(es_string_mayor_alfabeticamente(pokemon_nombre_maximo.nombre, pokedex->pokemons[i].nombre)){
                pokemon_nombre_maximo = pokedex->pokemons[i];
                posicion_maximo = i;
            }
        }
        pokedex->pokemons[posicion_maximo] = pokedex->pokemons[pokemons_no_ordenados - 1];
        pokedex->pokemons[pokemons_no_ordenados - 1] = pokemon_nombre_maximo;
...
bool pokedex_agregar_pokemon(struct pokedex *pokedex, struct pokemon pokemon)
{
    ...
    if(pokedex->cantidad_pokemons == pokedex->capacidad){
        size_t nueva_capacidad = (pokedex->capacidad == 0)? 1: (pokedex->capacidad * 2);

        struct pokemon *aux = (struct pokemon*) realloc(pokedex->pokemons, nueva_capacidad * sizeof(struct pokemon));
        if(aux == NULL){
        return false;
        }
